...
def run_beta_tests():
    ...
    # TESTE 14: Mal Condicionada
    print("\nTESTE 14: Matriz Mal Condicionada")
    A, b = generator.generate_ill_conditioned(n, condition_number = 1e5)
    results = benchmark.run_all_methods(A, b, max_iter = 2000)
    benchmark.print_comparison(results, "Mal Condicionada (κ = 1e5)")

    # TESTE 15: Não Simétrica com Dominância
    print("\nTESTE 15: Matriz Não Simétrica Diagonal Dominante")
    ...
    # TESTE 16: Indefinida (Simétrica)
    print("\nTESTE 16: Matriz Simétrica Indefinida")
    ...
    # TESTE 17: Autovalores em Clusters
    print("\nTESTE 17: Matriz SPD com Autovalores em Clusters")
    A, b = generator.generate_clustered_eigenvalues(n, num_clusters = 2, condition_number = 100)
    results = benchmark.run_all_methods(A, b)
    benchmark.print_comparison(results, "SPD com Clusters (2 clusters, κ = 100)")
    
    # TESTE 18: Toeplitz
    print("\nTESTE 18: Matriz de Toeplitz")
    A, b = generator.generate_toeplitz(n, alpha = 0.5)
    results = benchmark.run_all_methods(A, b)
    benchmark.print_comparison(results, f"Toeplitz Simétrica (a = 0.5, n = {n})")
    
    # TESTE 19: Laplaciana 2D
    print("\nTESTE 19: Matriz Laplaciana 2D (Discretização de Poisson)")
    m = int(np.sqrt(tamanho).round()) # Arredonda para o inteiro mais próximo
    A, b = generator.generate_laplacian_2d(m)
    results = benchmark.run_all_methods(A, b)
    benchmark.print_comparison(results, f"Laplaciana 2D (grid {m}x{m})")
    
    # TESTE 20: Matriz de Banda (Largura 5)
    print("\nTESTE 20: Matriz de Banda")
    A, b = generator.generate_band_matrix(n, bandwidth = 5)
    results = benchmark.run_all_methods(A, b)
    benchmark.print_comparison(results, f"Banda Simétrica (largura = 5, n = {n})")

if __name__ == "__main__":
    run_beta_tests()
    # generate_dataset(num_samples_per_type = 5, n_range = (10, 50), max_workers = 8, output_file = 'dataset.csv')